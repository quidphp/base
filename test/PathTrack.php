...
class PathTrack extends Base\Test
{
	...
	public static function trigger(array $data):bool
	{
		...
		assert(Base\PathTrack::build(array('dirname'=>'/test/ok','extension'=>'jpg')) === '/test/ok/.jpg');
		assert(Base\PathTrack::build(array('dirname'=>'test/ok','extension'=>'jpg')) === 'test/ok/.jpg');
		...
		assert(Base\PathTrack::extension(Base\PathTrack::change(array('extension'=>'jpg'),$_file_)) === 'jpg');
		assert('class.php' === Base\PathTrack::change(array('dirname'=>null),$_file_));
		assert('class.php' === Base\PathTrack::change(array('dirname'=>false),$_file_));
		assert('class.php' === Base\PathTrack::change(array('dirname'=>''),$_file_));
		assert('/class.php' === Base\PathTrack::change(array('dirname'=>'/'),$_file_));
		assert('a/class.php' === Base\PathTrack::change(array('dirname'=>'a'),$_file_));
		assert('/a/class.php' === Base\PathTrack::change(array('dirname'=>'/a'),$_file_));
		assert('.php' === Base\PathTrack::keep('extension',$_file_));
		assert('class' === Base\PathTrack::keep('filename',$_file_));
		assert('class.php' === Base\PathTrack::keep(array('filename','extension'),$_file_));
		assert(substr($_file_,0,-4) === Base\PathTrack::remove('extension',$_file_));
		assert('class' === Base\PathTrack::remove(array('dirname','extension'),$_file_));
		...
		assert(array("bla/bla","bla","") === Base\PathTrack::parents("bla/bla/bla"));
		assert(Base\Arr::valueLast(Base\PathTrack::parents($_file_)) === '');
		assert(Base\PathTrack::parents("bla") === array(''));
		...
		assert("/bla/bla/bla" === Base\PathTrack::separator("/bla//bla/bla"));
		assert("test/test2/test3" === Base\PathTrack::str(array("test","test2","test3")));
		assert("/test/test2/test3/" === Base\PathTrack::str("/test/test2/test3/"));
		assert("test/test2/test3/" === Base\PathTrack::str("test/test2/test3/",Base\PathTrack::option(array('end'=>true))));
		assert("test/test2/test3/" === Base\PathTrack::str("test/test2/test3/"));
		assert("test/test2/test3" === Base\PathTrack::str('test/test2//test3'));
		assert("/test/test2/test3/" === Base\PathTrack::str('//test/test2//test3/'));
		assert("" === Base\PathTrack::str(''));
		assert("bla/bla/bla" === Base\PathTrack::str("bla/bla/bla"));
		assert("bla/bla/bla" === Base\PathTrack::str(array("bla",'',"bla","bla")));
